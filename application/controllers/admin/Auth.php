...
{	
	...
	function index()
	{
		...
		else
		{
			...
			{					
				$LoginData = $this->AuthModel->login($_POST['email'], $_POST['password']);
	...
	function admin_profile()
	{
		if(isset($_POST['submit']))
		{
			...
			$data['admin_data'] = $this->AuthModel->admin_profile_update($data,$id);
		...
		{
			$data['admin_data'] 	= $this->AuthModel->admin_profile();
			$header_value = array(
	...
	function change_password()
	{
		...
		$data = $this->AuthModel->change_password($data);
		if($data)
		{
			$header_value = array(
		...
		{
			$this->session->set_flashdata("worng_message", "<div style = 'color:green;font-size: 18px;font-family: bold;'>Please Enter Correct Old Password</div>");
