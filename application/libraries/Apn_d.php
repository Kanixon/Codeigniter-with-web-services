	...
	class Apn_d
	{
		...
		*/
		function __construct() 
		{
			...
			$this->_ci->config->load('apn_d',true);
			
			if(!file_exists($this->_ci->config->item('PermissionFile','apn_d')))
			{
				show_error("APN Failed to connect: APN Permission file not found");
			}
			
			$this->pushServer = $this->_ci->config->item('Sandbox','apn_d') ? $this->_ci->config->item('PushGatewaySandbox','apn_d') : $this->_ci->config->item('PushGateway','apn_d');
		
			$this->feedbackServer = $this->_ci->config->item('Sandbox','apn_d') ? $this->_ci->config->item('FeedbackGatewaySandbox','apn_d') : $this->_ci->config->item('FeedbackGateway','apn_d');
			
			$this->keyCertFilePath = $this->_ci->config->item('PermissionFile','apn_d');

			$this->passphrase = $this->_ci->config->item('PassPhrase','apn_d');
			$this->timeout = $this->_ci->config->item('Timeout','apn_d') ? $this->_ci->config->item('Timeout','apn_d') : 60;
			$this->expiry = $this->_ci->config->item('Expiry','apn_d') ? $this->_ci->config->item('Expiry','apn_d') : 86400;
		}
		...
		*/
		...
		{ 
			//echo "<pre>";print_r($ids); echo $deviceToken; echo "<br>". $message;// die;
		...
		*/
		public function setData($data)
		{
			...
			if (isset($data['apn_d']))
			{
				log_message('error',"APN: cannot add additional data - key 'apn_d' is reserved");
				return false;
			}
